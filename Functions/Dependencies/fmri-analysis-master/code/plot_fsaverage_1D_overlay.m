function [color_data, patch_handle, light_handle] = ...
    plot_fsaverage_1D_overlay(surface_values, hemi, color_map_to_plot, color_range, figh, varargin)
...
nvertices = 163842; 
[vertices, faces] = freesurfer_read_surf([params('rootdir') 'myfsaverage/surf/' hemi '.inflated']);

% set default color data based on gyral/sulcal divisions
curv = read_curv([params('rootdir') 'myfsaverage/surf/' hemi '.curv']);
...
set(colorbar_handle, 'XTick', linspace(0,1,5), 'XTickLabel', ...
    colorbar_labels_str,'FontSize',20,'Position',[0.1469 0.05 0.7438 0.0312]);
